...
# A cursor enables manipulation of sets (it behaves as a generator in Python), it's preferable to be used instead
# of db.***, being a generator means that the data can only be retrieved once.
cursor = db.cursor()
...
cursor.execute("INSERT INTO contacts (name, phone, email) VALUES ('OJKA', 888883459, 'ojka@bldr.cat')")
cursor.execute("INSERT INTO contacts (name) VALUES ('Not me')")  # Record with null fields

# Using ? placeholder to insert, pass parameters in a tuple
name, num, email = 'Python', '010101', '01@01.com'
cursor.execute("INSERT INTO contacts (name, phone, email) VALUES (?,?,?)", (name, num, email))
...
users = [('name1', 'phone1', 'email1'),
         ('name2', 'phone2', 'email2'),
         ('name3', 'phone3', 'email3')]
cursor.executemany("INSERT INTO contacts (name, phone, email) VALUES(?,?,?)", users)
...
record = cursor.execute("SELECT * FROM contacts WHERE {} LIKE ?".format(field), (name,))
