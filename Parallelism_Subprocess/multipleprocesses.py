...
# poll() method serves as a run check - will return None if the process is
# still # running at the time poll() is called, if the process has terminated,
# it will return the process's integer exit code
print(calc_process.poll())

# wait() method will block main process until the launched process has
# terminated - the return value of wait() is the process's exit code
...
Popen().
To do so, pass a list as the sole argument, the first str in the list will be
the executable filename of the program to be launched; all the subsequent str
will be the command line arguments, in fact, this list will be the value of
sys.argv.
"""
...
# Programs in charge of searching for default application corresponding to
# file's extension
# Windows - 'start'
# OS X - 'open'
# Ubuntu Linux - 'see'
# subprocess.Popen(['see', 'sample.txt'])  # , shell=True) - needed only in
# Windows
