...
with open('binary2', 'bw') as binFile2:
    ...
    binFile2.write(b.to_bytes(2, 'big'))  # 'big' endian arranges the bytes from left to right
    binFile2.write(c.to_bytes(4, 'big'))
    binFile2.write(x.to_bytes(4, 'big'))
    binFile2.write(x.to_bytes(4, 'little'))  # 'little' endian arranges the bytes from right to left

with open('binary2', 'br') as binFile2:
    A = int.from_bytes(binFile2.read(2), 'big')
    print(A)
    B = int.from_bytes(binFile2.read(2), 'big')
    print(B)
    C = int.from_bytes(binFile2.read(4), 'big')
    print(C)
    X = int.from_bytes(binFile2.read(4), 'big')
    print(X)
    X2 = int.from_bytes(binFile2.read(4), 'big')
    print(X2)
