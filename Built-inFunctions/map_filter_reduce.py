...
The reason:
All lists returned by map() and filter() can be also created using list
comprehension.
"""
# map() - map() is in alternative to list comprehensions, being comprehensions
# faster
...
# map(@function, @iterable) - applies the @function to all the items in
# the @iterable
map_words = map(str.upper, text.split(' '))
# Returns a map object, it's an iterable
...
# filter(@function, @iterable) - applies @function to all the items
# in @iterable
# filter() applies a bool() over the return value of @function - so @function
# must return a boolean to be useful
# If the result is True, it's added to the filter list, discarded otherwise
meals_filter = filter(not_spam, menu)

# Returns a filter object, it's an iterable
